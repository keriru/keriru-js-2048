    ...
    for(let c = 0; c < columns; c++) {
        ...
        for(let r = 0; r < rows; r++){
            ...
            if(changedIndices.includes(r) && num !== 0){
            	tile.style.animation = "slide-from-buttom 0.3s"
            	setTimeout(()=>{
            		tile.style.animation ="";
            	}, 300);
            }
...
//capturing coordinate of the touch input
...
document.addEventListener("touchmove", (e) => {
    ...
    e.preventDefault(); //line scrolling disable
}, {passive: false});

//"touchend" listener
document.addEventListener("touchend", (e) => {

    //Checking the classname tile
    if(!e.target.className.includes("tile")){
        return;
    }

    //Calculating between horizontal and vertical of initial and final position
    let diffX = startX - e.changedTouches[0].clientX;
    let diffY = startY - e.changedTouches[0].clientY;

    //Checking the magnitude of the vertical swipe if the horizontal swipe is greater
